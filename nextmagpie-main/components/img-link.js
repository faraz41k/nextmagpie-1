...
const ImgLink = ({ propBackgroundImage, title, link }) => {
  ...
  return (
    <Link href={`/${link}`}
      ...
      style={card12Style}
    >
      <div className="self-stretch   flex flex-row items-center justify-center gap-[32px] backdrop-blur-sm bg-whitesmoke-200/20 rounded-lg">
        <div className="flex flex-row items-start justify-end gap-[8px]">
          <img
            ...
            src="/mappin@2x.png"
          />
          <div className="relative leading-[24px] font-medium p-1 ">{title}</div>
        </div>
        <div className="flex flex-row items-start justify-end gap-[8px]">
          <span></span>
